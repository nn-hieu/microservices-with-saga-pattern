...
public class RabbitMQConfig extends BaseRabbitMQConfig {
    ...
    // Queues
    @Bean
    public Queue userDebitSucceededQueue() {
        return QueueBuilder.durable(RabbitMQQueue.USER_DEBIT_SUCCEEDED_QUEUE.getValue())
    ...
    // Bindings
    @Bean
    public Binding userDebitSucceededBinding(Queue userDebitSucceededQueue, DirectExchange userExchange) {
        return BindingBuilder
                .bind(userDebitSucceededQueue)
                .to(userExchange)
                .with(UserEvent.USER_DEBIT_SUCCEEDED.getValue());
    }
